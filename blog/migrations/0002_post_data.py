# Generated by Django 3.1 on 2020-05-15 12:44
...
def add_post_data(apps, scheme_editor):
    ...
    for post in POSTS:
        ...
        post_object.pub_data = post['pub_date']
        post_object.save()
        for startup_slug in post['startups']:
            post_object.startups.add(Startup.objects.get(slug=startup_slug))

        for tag_slug in post['tags']:
            post_object.tags.add(Tag.objects.get(slug=tag_slug))



def remove_post_data(apps, scheme_editor):
    Post = apps.get_model('blog', 'Post')
    for post in POSTS:
